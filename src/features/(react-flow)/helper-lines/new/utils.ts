import { Box, InternalNode, NodeDimensionChange, NodePositionChange } from "@xyflow/react";
import { nodeToBox } from "@xyflow/system";
import { ANCHORS, SNAP_RADIUS } from "./config";
import { Anchor, AnchorMatch, CandidateLine, HelperLine, Orientation } from "./types";
import { PossibleNode } from "@/shared/react-flow/nodes/shared";

const isInViewport = (a: Box, b: Box) => a.x < b.x2 && a.x2 > b.x && a.y < b.y2 && a.y2 > b.y;
...
export class SpatialIndex {
  ...
  initialize(helperLines: HelperLine[]): void {
    ...
    this.yLines = helperLines.filter((line) => line.orientation === "horizontal");
  ...
  search(
    ...
    for (const line of lines) {
      ...
      const nodeDist = overlap > 0 ? 0 : boxDistance(line.nodeBox, nodeToBox(node));
    ...
    const lastLine = orientation === "horizontal" ? this.lastHorizontalLine : this.lastVerticalLine;
...
export function getHelperLines(
  ...
  node: PossibleNode,
  nodeBox: Box,
  validAnchors: (keyof typeof ANCHORS)[] = Object.keys(ANCHORS) as (keyof typeof ANCHORS)[],
  ...
  for (const anchorName of validAnchors) {
    ...
    const line = spatialIndex.search(anchor.orientation, pos, node, viewportBox);
  ...
  for (const anchorMatch of candidateAnchors) {
    ...
    const dist = Math.abs(anchorMatch.sourcePosition - anchorMatch.line.position);
    ...
    if (Math.abs(deltaY) <= SNAP_RADIUS) {
      ...
      if (dimensionChange?.dimensions) {
        if (positionChange) {
          ...
          const bottom = internalNode.internals.positionAbsolute.y + (node.measured?.height ?? 0);
    ...
    if (Math.abs(deltaX) <= SNAP_RADIUS) {
      ...
      if (dimensionChange?.dimensions) {
        if (positionChange) {
          ...
          const right = internalNode.internals.positionAbsolute.x + (node.measured?.width ?? 0);
