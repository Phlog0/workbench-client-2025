import { Box, useNodesInitialized, useReactFlow, useStore } from "@xyflow/react";
import { NodeChange, NodeDimensionChange, NodePositionChange, nodeToBox } from "@xyflow/system";
import { useCallback, useEffect, useRef, useState } from "react";
import { shallow } from "zustand/shallow";
// import { HelperLinesRenderer } from "./HelperLinesRenderer";
import { HelperLine } from "./types";
import { buildHelperLines, getHelperLines, snapToHelperLines, SpatialIndex } from "./utils";
...
export function useHelperLines() {
  const [helperLineHorizontal, setHelperLineHorizontal] = useState<HelperLine | undefined>(
    undefined,
  );
  const [helperLineVertical, setHelperLineVertical] = useState<HelperLine | undefined>(undefined);
  ...
  const { getNodes } = useReactFlow<PossibleNode, PossibleEdge>();
  ...
  const updateHelperLines = useCallback(
    (changes: NodeChange<PossibleNode>[], nodes: PossibleNode[]): NodeChange<PossibleNode>[] => {
      ...
      if (!spatialIndexRef.current || changes.length === 0 || changes.length > 2) {
        return changes;
      }
      ...
      const node = nodes.find((node) => node.id === (changes[0] as NodePositionChange).id);
      ...
      if (positionChange?.position && snapChange.position) {
        ...
        if (parentNode) {
          snapChange.position.x = positionChange.position.x + (parentNode?.position?.x ?? 0);
          snapChange.position.y = positionChange.position.y + (parentNode?.position?.y ?? 0);
        } else {
  ...
  //     return <HelperLinesRenderer horizontal={helperLineHorizontal} vertical={helperLineVertical} />;
